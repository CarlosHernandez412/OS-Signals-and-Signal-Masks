...
int main(void) {
   ...
   sigprocmask(SIG_BLOCK, &mask, NULL); /* block all signals until handler is
                                         * setup 
                                         */
   ...
   sigfillset(&sa.sa_mask);   /* sa.sa_mask is mask used in handler; previous
                               * process mask is restored upon handler exit; a
                               * full set means all signals are blocked - empty
                               * set means no signals are blocked; by default 
                               * the handler signal(s) are blocked */
   ...
   strcpy(buf,"Made it out of the loop!\n");
   write(1,buf,strlen(buf));
